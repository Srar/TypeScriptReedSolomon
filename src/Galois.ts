...
export default class Galois {

    /**
     * The number of elements in the field.
     */
    public static readonly FIELD_SIZE: number = 256;
     ...
     */
    public static readonly GENERATING_POLYNOMIAL: number = 29;
    ...
    public static readonly LOG_TABLE: number[] = [
    ...
    public static readonly EXP_TABLE: number[] = [
     ...
     */
    public static MULTIPLICATION_TABLE: Buffer[] = Galois.generateMultiplicationTable();
     ...
     */
    public static exp(a: number, n: number): number {
        if (n == 0) {
            return 1;
        }
        else if (a == 0) {
    ...
    public static multiply(a: number, b: number): number {
        if (a == 0 || b == 0) {
            ...
            let logA: number = Galois.LOG_TABLE[a & 0xFF];
            let logB: number = Galois.LOG_TABLE[b & 0xFF];
            let logResult: number = logA + logB;
            return Galois.EXP_TABLE[logResult];
        }
    }
     ...
     */
    public static generateMultiplicationTable(): Buffer[] {
        let result: Buffer[] = [];
        for (let a = 0; a < Galois.FIELD_SIZE; a++) {
            result[a] = Buffer.alloc(Galois.FIELD_SIZE);
            for (let b = 0; b < Galois.FIELD_SIZE; b++) {
                result[a][b] = Galois.multiply(a, b);
            }
        }
        return result;
    }
